...
public:
  ...
  void send_connection_setup_plain_header();
  void release_complete_tau(uint16_t rnti);
  void rrc_send_rrc_reconf(uint16_t rnti);
  
  void rrc_send_connection_setup(uint16_t rnti);
  void rrc_send_connection_setup_plain_header(uint16_t rnti);
  void rrc_send_rrc_reconf_downgraded(uint16_t rnti);
  bool setup_ue_ctxt(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUEST_STRUCT *msg);
    bool setup_ue_ctxt_replay(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTREPLAY_STRUCT  *msg);
  bool setup_ue_ctxt_downgraded(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTDOWNGRADED_STRUCT  *msg);
  void rrc_send_ue_capability_enquiry(uint16_t rnti);
  void rrc_send_connection_reestablish(uint16_t rnti);
  void rrc_send_connection_reestablish_plain_text(uint16_t rnti);
  
  void rrc_send_rrc_reconf_replay(uint16_t rnti);
  ...
  public:
    ...
    void send_connection_setup_plain_header(bool is_setup = true);
    void send_connection_setup(bool is_setup = true);
    void send_connection_reest();
    void send_connection_reest_plain_text();
    void send_connection_reject();
    void send_connection_release();
      void send_connection_release_tau();
    void send_connection_reest_rej();
    void send_connection_reest_req();
    void send_connection_reconf(srslte::unique_byte_buffer_t sdu);
      void send_connection_reconf_replay(srslte::unique_byte_buffer_t sdu);
    void send_connection_reconf_downgraded(srslte::unique_byte_buffer_t sdu);
    ...
    void send_security_mode_command();
      void send_security_mode_command_replay();
    ...
    void handle_rrc_con_reest_req(asn1::rrc::rrc_conn_reest_request_s* msg, srslte::unique_byte_buffer_t pdu);
    ...
    void set_security_key_plain(uint8_t* key, uint32_t length);
    ...
    void send_dl_dcch(asn1::rrc::dl_dcch_msg_s*    dl_dcch_msg,
      ...
      void send_dl_dcch_replay(asn1::rrc::dl_dcch_msg_s*    dl_dcch_msg,
                        srslte::unique_byte_buffer_t pdu = srslte::unique_byte_buffer_t());
...
private:
  ...
  void disable_integrity(uint16_t rnti, uint32_t lcid);
  void disable_encryption(uint16_t rnti, uint32_t lcid);
  srslte::byte_buffer_pool* pool;
  ...
  const static uint32_t LCID_REL_USER3 = 0xffff0006;
