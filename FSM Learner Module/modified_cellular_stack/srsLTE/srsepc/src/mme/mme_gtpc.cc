  ...
  switch (pdu->header.type) {
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE:
      ...
      case srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_TAU:
          ...
          break;
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_REPLAY:
      handle_create_session_response_replay(pdu);
      break;
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_DOWNGRADED:
      handle_create_session_response_downgraded(pdu);
      break;
...
bool mme_gtpc::send_create_session_request_replay(uint64_t imsi)
{
  ...
  cs_req_pdu.header.type = srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST_REPLAY;
...
bool mme_gtpc::send_create_session_request_downgraded(uint64_t imsi)
{
  m_mme_gtpc_log->info("Sending Create Session Request.\n");
  m_mme_gtpc_log->console("Sending Create Session Request.\n");
  struct srslte::gtpc_pdu                     cs_req_pdu;
  ...
  cs_req_pdu.header.teid_present = true;
  cs_req_pdu.header.teid         = 0; // Send create session request to the butler TEID
  cs_req_pdu.header.type = srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST_DOWNGRADED;
...
bool mme_gtpc::handle_create_session_response_replay(srslte::gtpc_pdu* cs_resp_pdu)
{
  struct srslte::gtpc_create_session_response* cs_resp = &cs_resp_pdu->choice.create_session_response;
  m_mme_gtpc_log->info("Received Create Session Response Replay\n");
  m_mme_gtpc_log->console("Received Create Session Response Replay\n");
  if (cs_resp_pdu->header.type != srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_REPLAY) {
    m_mme_gtpc_log->warning("Could not create GTPC session. Not a create session response\n");
    // TODO Handle error
    return false;
  }
  ...
  esm_ctx->pdn_addr_alloc   = cs_resp->paa;
  esm_ctx->sgw_s1u_fteid    = cs_resp->eps_bearer_context_created.s1_u_sgw_f_teid;
  m_s1ap->m_s1ap_ctx_mngmt_proc->send_initial_context_setup_request_replay(nas_ctx, default_bearer);
  return true;
}


bool mme_gtpc::handle_create_session_response_downgraded(srslte::gtpc_pdu* cs_resp_pdu)
{
  struct srslte::gtpc_create_session_response* cs_resp = &cs_resp_pdu->choice.create_session_response;
  m_mme_gtpc_log->info("Received Create Session Response Replay\n");
  m_mme_gtpc_log->console("Received Create Session Response Replay\n");
  if (cs_resp_pdu->header.type != srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_DOWNGRADED) {
    m_mme_gtpc_log->warning("Could not create GTPC session. Not a create session response\n");
    // TODO Handle error
    return false;
  }
  ...
  esm_ctx->pdn_addr_alloc   = cs_resp->paa;
  esm_ctx->sgw_s1u_fteid    = cs_resp->eps_bearer_context_created.s1_u_sgw_f_teid;
  m_s1ap->m_s1ap_ctx_mngmt_proc->send_initial_context_setup_request_downgraded(nas_ctx, default_bearer);
  return true;
}
