  ...
  switch (pdu->header.type) {
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST:
      ...
      case srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST_TAU:
          ...
          break;
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST_REPLAY:
      handle_create_session_request_replay(pdu->choice.create_session_request);
      break;
    case srslte::GTPC_MSG_TYPE_CREATE_SESSION_REQUEST_DOWNGRADED:
      handle_create_session_request_downgraded(pdu->choice.create_session_request);
      break;
...
void spgw::gtpc::handle_create_session_request_replay(const struct srslte::gtpc_create_session_request& cs_req)
{
  m_gtpc_log->info("SPGW Received Create Session Request\n");
  spgw_tunnel_ctx_t* tunnel_ctx;
  int                default_bearer_id = 5;
  ...
  header->teid_present = true;
  header->teid         = tunnel_ctx->dw_ctrl_fteid.teid; // Send create session response to the UE's MME Ctrl TEID
  header->type         = srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_REPLAY;
  ...
  cs_resp->paa.ipv4_present = true;
  cs_resp->paa.ipv4         = tunnel_ctx->ue_ipv4;
  m_gtpc_log->console("Sending Create Session Response Replay\n");

  // Send Create session response to MME
  send_s11_pdu(cs_resp_pdu);
  return;
}
void spgw::gtpc::handle_create_session_request_downgraded(const struct srslte::gtpc_create_session_request& cs_req)
{
  m_gtpc_log->info("SPGW Received Create Session Request\n");
  spgw_tunnel_ctx_t* tunnel_ctx;
  int                default_bearer_id = 5;
  ...
  header->teid_present = true;
  header->teid         = tunnel_ctx->dw_ctrl_fteid.teid; // Send create session response to the UE's MME Ctrl TEID
  header->type         = srslte::GTPC_MSG_TYPE_CREATE_SESSION_RESPONSE_DOWNGRADED;
