...
public:
  ...
  int  enb_listen();
  int  init(s1ap_args_t s1ap_args, srslte::log_filter* s1ap_log, srslte::log_filter* nas_log, mme_statelearner_interface_s1ap *mme_statelearner_); // Fuzzing
  void stop();
  ...
  //bool send_fuzzing_command_to_enodeb(nas* nas_ctx);
