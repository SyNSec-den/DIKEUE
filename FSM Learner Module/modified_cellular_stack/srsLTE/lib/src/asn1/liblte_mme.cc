                                                       ...
                                                       LIBLTE_MME_TAU_REQUEST_MSG_STRUCT* tau_req)
{
  ...
  if (msg != NULL && tau_req != NULL) {
    ...
    liblte_mme_unpack_esm_message_container_ie(&msg_ptr, &tau_req->esm_msg);
    ...
    if (LIBLTE_MME_MS_CLASSMARK_3_IEI == *msg_ptr) {
      msg_ptr++;
      liblte_mme_unpack_mobile_station_classmark_3_ie(&msg_ptr, &tau_req->ms_cm3);
                                           ...
                                           LIBLTE_BYTE_MSG_STRUCT*                       msg)
{
  ...
  if (dl_nas_transport != NULL && msg != NULL) {
    ...
    *msg_ptr = LIBLTE_MME_MSG_TYPE_DOWNLINK_NAS_TRANSPORT;
    msg_ptr++;
    ///IK
    //*msg_ptr = ( LIBLTE_MME_MSG_TRANSITION_IDENTIFIER_CP_DATA << 4) |LIBLTE_MME_MSG_PROTOCOL_DISCRIMINATOR_CP_DATA;
    //msg_ptr++;


    //*msg_ptr = LIBLTE_MME_MSG_TYPE;
    //msg_ptr++;
...
LIBLTE_ERROR_ENUM liblte_mme_pack_identity_request_msg_mac(LIBLTE_MME_ID_REQUEST_MSG_STRUCT* id_req,
                                                       LIBLTE_BYTE_MSG_STRUCT*  msg,uint32 count,  uint8    sec_hdr_type)
{
    LIBLTE_ERROR_ENUM err     = LIBLTE_ERROR_INVALID_INPUTS;
    uint8*            msg_ptr = msg->msg;
