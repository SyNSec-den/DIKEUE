        ...
        for field in node_list:
            ...
            if "UE_NAS1_" in field:
                ...
                if('true' in value.lower()):
                    ...
                    tlabel = "UE_NAS1_T" + str(label_t1)
                    #print tlabel, value

            if "UE_NAS2_" in field:
                remaining = field.split("T",1)[1] 
                ...
                if ('true' in value.lower()):
                    ...
                    tlabel = "UE_NAS2_T" + str(label_t2)
                    #print tlabel, value


            if "ue_nas1_action" in field:
                action1 = field.split("= ",1)[1].strip()
                #print 'action 1 = ', action1

            if "ue_nas2_action" in field:
...
def compute_diff(nuxmfilename, commandfilename, outputfilename, input_condition = None, fsm1_action=None, fsm2_action=None):
    output_list = []
    ...
    if not is_ltlspec_present:
        ltlspec =  "\n\nLTLSPEC ( \n G( ! (input = "+ input_condition + " & ue_nas1_action = " + fsm1_action + " & ue_nas2_action = " + fsm2_action + "))\n);"
        print("LTL Property:")
        print (ltlspec)
    ...
    while True:
        ...
        for state in states:
            ...
            if state.action1 == fsm1_action and state.action2 == fsm2_action:               
        ...
        for i in range(0, len(nuxmv)):
            ...
            if "LTLSPEC (" in nuxmv[i] and iter == 0:
                ...
                if (offending_label_t1):
                    invariant = "G(!UE_NAS1_T" + offending_label_t1
                if (offending_label_t1 and offending_label_t2):
                    invariant = invariant + " & "+"!UE_NAS2_T" + offending_label_t2 
                if (not offending_label_t1 and offending_label_t2):
                    invariant = invariant + "G(!UE_NAS2_T" + offending_label_t2
                invariant = invariant + ")" + "\n" + "->" + "\n"
                #print invariant
                ...
                if (offending_label_t1):
                    nuxmv[i] = first + " & !UE_NAS1_T" + offending_label_t1
                if (offending_label_t2):
                    nuxmv[i] = first + " & !UE_NAS2_T" + offending_label_t2
                nuxmv[i] = nuxmv[i] + ")" + "\n"
