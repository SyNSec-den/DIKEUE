            ...
            for act in acts:
                ...
                if action_label in out_msgs:
                    if 'ue' in fsm_label.lower() and 'rrc' in fsm_label.lower():
                        ...
                        chan_start = 'BS_RRC'
                        chan_end = 'UE_RRC'
                    elif 'mme' in fsm_label.lower() and 'nas' in fsm_label.lower():
                        chan_label = 'chan_MU'
                        chan_start = 'MME_NAS'
                        chan_end = 'UE_NAS'
            ...
            for token in cond_tokens:
                ...
                if token in in_msgs:
                    ...
                    if 'ue' in fsm_label.lower() and 'rrc' in fsm_label.lower():
                        ...
                        msg_prefix = 'chan_MU = chanMU_'
                    elif 'bs' in fsm_label.lower() and 'rrc' in fsm_label.lower():
                        msg_prefix = 'chan_UB=chanUB_'
                    elif 'mme' in fsm_label.lower() and 'nas' in fsm_label.lower():
                        msg_prefix = 'chan_UM=chanUM_'
...
def main(fsm1_inFile, fsm2_inFile, outputFile):
    ...
    if 'ue' in fsm1_inFile.lower():
        if 'rrc' in fsm1_inFile.lower():
            fsm1_label = 'UE_RRC1'

        elif 'nas' in fsm1_inFile.lower():
            fsm1_label = 'UE_NAS1'


    # FSM 2
    if 'ue' in fsm2_inFile.lower():
        #model = fsm1_inFile.lower().split('_')[0]
        if 'rrc' in fsm2_inFile.lower():
            fsm2_label = 'UE_RRC2'

        elif 'nas' in fsm2_inFile.lower():
            fsm2_label = 'UE_NAS2'

    #print fsm1_label, fsm2_label

    fsm1 = parseDOT(fsm1_inFile, fsm1_label) # return (fsm, env_vars)
    fsm2 = parseDOT(fsm2_inFile, fsm2_label) # return (fsm, env_vars)

    #fsm1 = parsing_result_fsm1[0]
    #fsm2 = parsing_result_fsm2[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lts1', dest='lts1_dot_file', default="FSM-real/nexus6-ue-nas.dot", help="UE NAS LTS")
    parser.add_argument('-lts2', dest='lts2_dot_file', default="FSM-real/iphone10-ue-nas.dot",
                        help="AMF NAS dot file to read")
    parser.add_argument('-o', dest='outputFile', default="EQCHECK-NAS-REAL.smv", help="smv file to write")

    args = parser.parse_args()
    main(args.lts1_dot_file, args.lts2_dot_file, args.outputFile)
