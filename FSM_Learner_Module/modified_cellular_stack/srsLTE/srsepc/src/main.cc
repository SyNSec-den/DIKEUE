...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("mme.ue_under_test_imsi", bpo::value<string>(&ue_under_test_imsi)->default_value("001011234567895"),  "Set UE_UNDER_TEST_IMSI for protocol state fuzzing")
    ("mme.ue_state_fuzzing_enable",   bpo::value<bool>(&args->mme_args.s1ap_args.ue_state_fuzzing_enable)->default_value(true),         "Enable UE_STATE_FUZZING")
    ...
    ("pcap.enable",   bpo::value<bool>(&args->mme_args.s1ap_args.pcap_enable)->default_value(true),         "Enable S1AP PCAP")
