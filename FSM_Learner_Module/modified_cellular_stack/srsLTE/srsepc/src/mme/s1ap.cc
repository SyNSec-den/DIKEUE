...
int s1ap::init(s1ap_args_t s1ap_args, srslte::log_filter* nas_log, srslte::log_filter* s1ap_log, mme_statelearner_interface_s1ap *mme_statelearner_) 
{
...
bool s1ap::expire_nas_timer(enum nas_timer_type type, uint64_t imsi)
{
  ...
  return err;
    ...
    bool s1ap::send_query(uint8_t *msg, uint16_t len) {
        ...
        else if(memcmp(msg, "enable_attach\n", len) == 0){
            pthread_mutex_lock(&s1ap_instance_mutex);

            m_s1ap_log->console("###query: enable_attach ###\n");
        ...
        else if(memcmp(msg, "auth_request_plain_text\n", len) == 0){
            m_s1ap_log->console("###query: auth_request_plain_text ###\n");
        ...
        else if(memcmp(msg, "identity_request_plain_text\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_plain_text ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST);
        }
        
        else if(memcmp(msg, "identity_request_imei\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_imei ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_IMEI);
        }
        else if(memcmp(msg, "identity_request_guti\n", len) == 0){
            m_s1ap_log->console("###query: identity_request_guti ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_IDENTITY_REQUEST_GUTI);
        }
        ...
        else if(memcmp(msg, "sm_command_protected\n", len) == 0){
            m_s1ap_log->console("### query: sm_command_protected ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD);
        }
        else if(memcmp(msg, "sm_command_null_security\n", len) == 0){
          m_s1ap_log->console("### query: sm_command_null_security ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD_NS);
        }
        else if(memcmp(msg, "sm_command_null_security_replay\n", len) == 0){
          m_s1ap_log->console("### query: sm_command_null_security_replay ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD_NS_REPLAY);
        }

        else if(memcmp(msg, "security_mode_command_no_integrity\n", len) == 0){
          m_s1ap_log->console("### query: security_mode_command_no_integrity ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD_NO_INTEGRITY);
        }
        else if(memcmp(msg, "sm_command_plain_text\n", len) == 0){
          m_s1ap_log->console("### query: sm_command_plain_text ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD_PLAIN);
        }
        else if(memcmp(msg, "attach_accept_protected\n", len) == 0){
            m_s1ap_log->console("### query: single attach_accept_protected ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_SINGLE);
        }
        else if(memcmp(msg, "attach_accept_no_integrity\n", len) == 0){
          m_s1ap_log->console("### query: single attach_accept_no_integrity ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_SINGLE_NO_INTEGRITY);
        }
        else if(memcmp(msg, "attach_accept_null_header\n", len) == 0){
          m_s1ap_log->console("### query: single attach_accept_null_header ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_SINGLE_NULL_HEADER);
        }
        else if(memcmp(msg, "RRC_sm_command_protected\n", len) == 0){
            m_s1ap_log->console("### query: RRC_sm_command_protected ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT);
        }
        else if(memcmp(msg, "RRC_sm_command_replay\n", len) == 0){
          m_s1ap_log->console("### query: RRC_sm_command_replay ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_RRC_SECURITY_MODE_COMMAND_REPLAY);
        }
        else if(memcmp(msg, "RRC_sm_command_null_security\n", len) == 0){
          m_s1ap_log->console("### query: RRC_sm_command_null_security ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_RRC_SECURITY_MODE_COMMAND_DOWNGRADED);
        }
        else if(memcmp(msg, "tau_accept_protected\n", len) == 0){
            m_s1ap_log->console("### query: tau_accept_protected ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_TAU_ACCEPT);
        }
        else if(memcmp(msg, "tau_accept_plain_header\n", len) == 0){
          m_s1ap_log->console("### query: tau_accept_plain_header ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_TAU_ACCEPT_PLAIN);
        }
        
        else if(memcmp(msg, "emm_info_plain\n", len) == 0){
            m_s1ap_log->console("###query: emm_info_plain ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_EMM_INFO_PLAIN);
        }
        else if(memcmp(msg, "service_reject\n", len) == 0){
            m_s1ap_log->console("###query: service_reject ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SERVICE_REJECT);
        }
        else if(memcmp(msg, "tau_reject\n", len) == 0){
            m_s1ap_log->console("###query: tau reject ###\n");
            ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_TAU_REJECT);
        }
        ...
        else if(memcmp(msg, "DL_NAS_transport_protected\n", len) == 0){
            m_s1ap_log->console("###query: DL_NAS_transport_protected ###\n");
        ...
        else if(memcmp(msg, "GUTI_reallocation_protected\n", len) == 0){
        ...
        else if(memcmp(msg,"sm_command_replay\n",len)==0){
          m_s1ap_log->console("### query: sm_command_replay ###\n");
        ...
        else if(memcmp(msg, "enable_tau\n", len) == 0){

            m_s1ap_log->console("### query: enable_tau ###\n");
            nas* nas_ctx = find_nas_ctx_from_imsi(m_s1ap_args.ue_under_test_imsi);
            if (nas_ctx == NULL) {
                m_s1ap_log->error("Error finding NAS context when sending UE Context Setup Release\n");
                return false;
        ...
        else if(memcmp(msg, "paging\n", len) == 0){

          m_s1ap_log->console("### query: paging ###\n");
          m_s1ap_paging->send_paging(m_s1ap_args.ue_under_test_imsi, 0);
          ret = true;

        }
        else if(memcmp(msg, "paging_imsi\n", len) == 0){

          m_s1ap_log->console("### query: paging_imsi ###\n");
          m_s1ap_paging->send_paging_imsi(m_s1ap_args.ue_under_test_imsi, 0);
          ret = true;

        }
        else if(memcmp(msg, "paging_tmsi_imsi\n", len) == 0){

          m_s1ap_log->console("### query: paging_tmsi_imsi ###\n");
          m_s1ap_paging->send_paging(m_s1ap_args.ue_under_test_imsi, 0);
          m_s1ap_paging->send_paging_imsi(m_s1ap_args.ue_under_test_imsi, 0);
          ret = true;

        }
        else if(memcmp(msg, "paging_no_id\n", len) == 0){
        ...
        else if(memcmp(msg, "attach_accept_plain_text\n", len) == 0){
          m_s1ap_log->console("### query: single attach_accept_plain_text ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_ATTACH_ACCEPT_SINGLE_NO_INTEGRITY);
         
        }

        else if(memcmp(msg, "sm_command_plain_header\n", len) == 0){
          m_s1ap_log->console("### query: sm_command_plain_header ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_SEC_MOD_CMD_PLAIN);
         
        }

        else if(memcmp(msg, "RRC_sm_command_plain_text\n", len) == 0){
          m_s1ap_log->console("### query: RRC_sm_command_plain_text ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_RRC_SECURITY_MODE_COMMAND_DOWNGRADED);
         
        }

        else if(memcmp(msg, "RRC_sm_command_plain_header\n", len) == 0){
          m_s1ap_log->console("### query: RRC_sm_command_plain_text ###\n");
          ret = m_s1ap_nas_transport->handle_statelearner_query(FUZZING_MSG_TYPE_RRC_SECURITY_MODE_COMMAND_DOWNGRADED);
