...
bool s1ap_paging::send_paging_imsi(uint64_t imsi, uint16_t erab_to_setup) {
  m_s1ap_log->info("Preparing to Page UE -- IMSI %015" PRIu64 "\n", imsi);
  ...
  paging->UEPagingID.choice_type = LIBLTE_S1AP_UEPAGINGID_CHOICE_IMSI;
  paging->UEPagingID.choice.iMSI.n_octets = 15;
  paging->UEPagingID.choice.iMSI.buffer[0] = 0;
  paging->UEPagingID.choice.iMSI.buffer[1] = 0;
  paging->UEPagingID.choice.iMSI.buffer[2] = 1;
  paging->UEPagingID.choice.iMSI.buffer[3] = 0;
  paging->UEPagingID.choice.iMSI.buffer[5] = 1;
  paging->UEPagingID.choice.iMSI.buffer[6] = 2;
  paging->UEPagingID.choice.iMSI.buffer[7] = 3;
  paging->UEPagingID.choice.iMSI.buffer[8] = 4;
  paging->UEPagingID.choice.iMSI.buffer[9] = 5;
  paging->UEPagingID.choice.iMSI.buffer[10] = 6;
  paging->UEPagingID.choice.iMSI.buffer[11] = 7;
  paging->UEPagingID.choice.iMSI.buffer[12] = 8;
  paging->UEPagingID.choice.iMSI.buffer[13] = 9;
  paging->UEPagingID.choice.iMSI.buffer[14] = 5;
  ...
  paging->TAIList.buffer[0].ext = false;
  paging->TAIList.buffer[0].tAI.ext = false;
  uint32_t plmn = htonl(m_s1ap->get_plmn());
  paging->TAIList.buffer[0].tAI.pLMNidentity.buffer[0] = ((uint8_t *) &plmn)[1];
  paging->TAIList.buffer[0].tAI.pLMNidentity.buffer[1] = ((uint8_t *) &plmn)[2];
  paging->TAIList.buffer[0].tAI.pLMNidentity.buffer[2] = ((uint8_t *) &plmn)[3];
  uint16_t tac = htons(m_s1ap->m_s1ap_args.tac); 
  memcpy(paging->TAIList.buffer[0].tAI.tAC.buffer, &tac, sizeof(uint16_t));
  paging->TAIList.buffer[0].tAI.iE_Extensions_present = false;
  paging->TAIList.buffer[0].iE_Extensions_present = false;
  ...
  // Send Paging to eNBs
  m_s1ap_log->info("Paging UE -- IMSI :0x%x\n", imsi);
  m_s1ap_log->console("###### Paging UE -- M-TMSI :0x%x ####\n", imsi);
  srslte::byte_buffer_t* reply_buffer = m_pool->allocate();
  LIBLTE_ERROR_ENUM      err          = liblte_s1ap_pack_s1ap_pdu(&pdu, (LIBLTE_BYTE_MSG_STRUCT*)reply_buffer);
  if (err != LIBLTE_SUCCESS) {
