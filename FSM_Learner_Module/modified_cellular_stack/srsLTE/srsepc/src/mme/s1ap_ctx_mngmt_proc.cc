...
namespace srsepc {
    ...
    bool s1ap_ctx_mngmt_proc::send_initial_context_setup_request(nas* nas_ctx, uint16_t erab_to_setup)
    {
      ...
      printf("meh meh normal intiating message: %s\n", liblte_s1ap_initiatingmessage_choice_text[init->choice_type]);
        ...
        m_s1ap_log->console("Did not attach accept to Initial Context Setup Request\n");
...
bool s1ap_ctx_mngmt_proc::send_initial_context_setup_request_replay(nas* nas_ctx, uint16_t erab_to_setup)
{
  // Prepare reply PDU
  printf("Reached send_initial_context_setup_request_replay\n");
  LIBLTE_S1AP_S1AP_PDU_STRUCT pdu;
  bzero(&pdu, sizeof(LIBLTE_S1AP_S1AP_PDU_STRUCT));
  pdu.choice_type = LIBLTE_S1AP_S1AP_PDU_CHOICE_INITIATINGMESSAGE;
  LIBLTE_S1AP_INITIATINGMESSAGE_STRUCT* init = &pdu.choice.initiatingMessage;
  init->procedureCode                        = LIBLTE_S1AP_PROC_ID_INITIALCONTEXTSETUP_REPLAY;
  init->choice_type                          = LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_INITIALCONTEXTSETUPREQUEST_REPLAY;

  LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTREPLAY_STRUCT* in_ctxt_req = &init->choice.InitialContextSetupRequestReplay;
  m_s1ap_log->info("Preparing to send Initial Context Setup request\n");
  printf("meh meh intiating message: %s\n", liblte_s1ap_initiatingmessage_choice_text[init->choice_type]);
  // Get UE Context/E-RAB Context to setup
  emm_ctx_t* emm_ctx = &nas_ctx->m_emm_ctx;
  ecm_ctx_t* ecm_ctx = &nas_ctx->m_ecm_ctx;
  esm_ctx_t* esm_ctx = &nas_ctx->m_esm_ctx[erab_to_setup];
  ...
  if (emm_ctx->state == EMM_STATE_DEREGISTERED) { 
    // Attach procedure initiated from an attach request
    m_s1ap_log->console("Adding tau accept to Initial Context Setup Request\n");
  ...
  m_pool->deallocate(reply_buffer);
  m_pool->deallocate(nas_buffer);
  printf("Leaving send_initial_context_setup_request_replay\n");
  return true;
}
bool s1ap_ctx_mngmt_proc::send_initial_context_setup_request_downgraded(nas* nas_ctx, uint16_t erab_to_setup)
{
  // Prepare reply PDU
  printf("Reached send_initial_context_setup_request_downgraded\n");
  LIBLTE_S1AP_S1AP_PDU_STRUCT pdu;
  bzero(&pdu, sizeof(LIBLTE_S1AP_S1AP_PDU_STRUCT));
  pdu.choice_type = LIBLTE_S1AP_S1AP_PDU_CHOICE_INITIATINGMESSAGE;
  LIBLTE_S1AP_INITIATINGMESSAGE_STRUCT* init = &pdu.choice.initiatingMessage;
  init->procedureCode                        = LIBLTE_S1AP_PROC_ID_INITIALCONTEXTSETUP_DOWNGRADED;
  init->choice_type                          = LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_INITIALCONTEXTSETUPREQUEST_DOWNGRADED;

  LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTDOWNGRADED_STRUCT* in_ctxt_req = &init->choice.InitialContextSetupRequestDowngraded;
  m_s1ap_log->info("Preparing to send Initial Context Setup request\n");
  printf("meh meh intiating message: %s\n", liblte_s1ap_initiatingmessage_choice_text[init->choice_type]);
  // Get UE Context/E-RAB Context to setup
  emm_ctx_t* emm_ctx = &nas_ctx->m_emm_ctx;
