...
public:
  ...
  virtual void rem_user(uint16_t rnti) = 0;
  virtual void write_sdu(uint16_t rnti, uint32_t lcid, srslte::unique_byte_buffer_t sdu)   = 0;
  virtual void write_sdu_replay(uint16_t rnti, uint32_t lcid, srslte::unique_byte_buffer_t sdu)   = 0;
...
public:
  ...
  virtual bool setup_ue_ctxt_replay(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTREPLAY_STRUCT  *msg) = 0;
  virtual bool setup_ue_ctxt_downgraded(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTDOWNGRADED_STRUCT  *msg) = 0;
  ...
  virtual void rrc_send_rrc_reconf(uint16_t rnti) = 0;
  virtual void rrc_send_rrc_reconf_replay(uint16_t rnti) = 0;
  virtual void rrc_send_connection_setup(uint16_t rnti) = 0;
  virtual void rrc_send_connection_setup_plain_header(uint16_t rnti) = 0;
  virtual void rrc_send_rrc_reconf_downgraded(uint16_t rnti) = 0;
  virtual void rrc_send_ue_capability_enquiry(uint16_t rnti) = 0;
  virtual void rrc_send_connection_reestablish(uint16_t rnti) = 0;
  virtual void rrc_send_connection_reestablish_plain_text(uint16_t rnti) = 0;
...
public:
  ...
  virtual bool notify_response(uint8_t *response, uint16_t size) = 0;
};
