...
void pdcp_entity::write_sdu_replay(unique_byte_buffer_t sdu, bool blocking)
{
  printf("in write_sdu_replay\n");
  uint32_t tx_count_prev      = 0;
  ...
  printf("TX %s SDU, SN: %d, do_integrity = %s, do_encryption = %s\n",
         rrc->get_rb_name(lcid).c_str(), tx_count,
         (do_integrity) ? "true" : "false", (do_encryption) ? "true" : "false");
  ...
  if (cfg.is_control) {
    ...
    if(do_integrity) {
      printf("Went into integrity generation\n");
  ...
  tx_count = tx_count_prev;
  tx_count++;
  printf("Sending the replayed message!!\n");
...
void pdcp_entity::disable_integrity()
{
  do_integrity = false;
}

void pdcp_entity::disable_encryption()
{
  do_encryption = false;
}
// RLC interface
...
void pdcp_pack_control_pdu(uint32_t sn, byte_buffer_t *sdu)
{
  ...
  *sdu->msg = sn & 0x1F;
  ...
  //printf("value check %u\n",(PDCP_CONTROL_MAC_I >> 24) & 0xFF);
  //printf("value check %u\n",(PDCP_CONTROL_MAC_I >> 16) & 0xFF);
  //printf("value check %u\n",(PDCP_CONTROL_MAC_I >> 8) & 0xFF);
  //printf("value check %u\n",PDCP_CONTROL_MAC_I);
