...
bool s1ap::init(s1ap_args_t args_, rrc_interface_s1ap *rrc_, enodeb_statelearner_interface_s1ap *enodeb_statelearner_, srslte::log *s1ap_log_) 
{
  rrc = rrc_;
  args = args_;
...
bool s1ap:: execute_command(uint8_t* msg, uint16_t len){
  ...
  if(memcmp(msg, "rrc_reconf\n", len) == 0) {
    s1ap_log->console("rrc_reconf\n");
    rrc->rrc_send_rrc_reconf(0);
  }
  if(memcmp(msg, "RRC_reconf_replay\n", len) == 0) {
    s1ap_log->console("RRC_reconf_replay\n");
    rrc->rrc_send_rrc_reconf_replay(0);
  }
  if(memcmp(msg, "rrc_reconf_downgraded\n", len) == 0) {
    s1ap_log->console("rrc_reconf_downgraded\n");
    rrc->rrc_send_rrc_reconf_downgraded(0);
  }


  if(memcmp(msg, "RRC_connection_setup_plain_text\n", len) == 0) {
    s1ap_log->console("RRC_connection_setup_plain_text\n");
    rrc->rrc_send_connection_setup(0);    
  }

  if(memcmp(msg, "RRC_connection_setup_plain_header\n", len) == 0) {
    s1ap_log->console("RRC_connection_setup_plain_header\n");
    rrc->rrc_send_connection_setup_plain_header(0);  
  }

  if(memcmp(msg, "RRC_reconf_plain_text\n", len) == 0) {
    s1ap_log->console("RRC_reconf_plain_text\n");
    rrc->rrc_send_rrc_reconf_downgraded(0);    
  }

  if(memcmp(msg, "RRC_con_reest_protected\n", len) == 0) {
    s1ap_log->console("RRC_con_reest_protected\n");
    rrc->rrc_send_connection_reestablish(0);    
  }

  if(memcmp(msg, "RRC_con_reest_plain_text\n", len) == 0) {
    s1ap_log->console("RRC_con_reest_plain_text\n");
    rrc->rrc_send_connection_reestablish_plain_text(0); 
  }

  if(memcmp(msg, "RRC_ue_info_req_protected\n", len) == 0) {
    s1ap_log->console("RRC_ue_info_req_protected\n");
    rrc->rrc_send_ue_capability_enquiry(0);  
  }
...
{
  printf("intiating message: %s\n", liblte_s1ap_initiatingmessage_choice_text[msg->choice_type]);
  switch(msg->choice_type) {
    case  LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_INITIALCONTEXTSETUPREQUEST_REPLAY:
      return handle_initialctxtsetuprequest_replay(&msg->choice.InitialContextSetupRequestReplay);
    case  LIBLTE_S1AP_INITIATINGMESSAGE_CHOICE_INITIALCONTEXTSETUPREQUEST_DOWNGRADED:
      return handle_initialctxtsetuprequest_downgraded(&msg->choice.InitialContextSetupRequestDowngraded);
...
{
  s1ap_log->console("Received InitialContextSetupRequest\n");
  if(msg->ext) {
    s1ap_log->warning("Not handling S1AP message extension\n");
  }
  if(enbid_to_rnti_map.end() == enbid_to_rnti_map.find(msg->eNB_UE_S1AP_ID.ENB_UE_S1AP_ID)) {
...
bool s1ap::handle_initialctxtsetuprequest_replay(LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTREPLAY_STRUCT *msg)
{
  printf("In handle_initialctxtsetuprequest_replay\n");
  ...
  // Setup UE ctxt in RRC
  if(!rrc->setup_ue_ctxt_replay(rnti, msg)) {
    return false;
  }

  return true;
}
bool s1ap::handle_initialctxtsetuprequest_downgraded(LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTDOWNGRADED_STRUCT *msg)
{
  printf("In handle_initialctxtsetuprequest_downgraded\n");
  s1ap_log->info("Received InitialContextSetupRequest\n");
  if(msg->ext) {
    s1ap_log->warning("Not handling S1AP message extension\n");
  }
  ...
  // Setup UE ctxt in RRC
  if(!rrc->setup_ue_ctxt_downgraded(rnti, msg)) {
    return false;
  }
