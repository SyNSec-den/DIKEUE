...
void rrc::rrc_send_rrc_reconf(uint16_t rnti2){
  if (rnti_last != 0) {
    srslte::unique_byte_buffer_t pdu2 = srslte::allocate_unique_buffer(*pool);
    users[rnti_last].send_connection_reconf(std::move(pdu2));
  }

}
void rrc::rrc_send_rrc_reconf_replay(uint16_t rnti2){
  if(rnti_last != 0) {
    srslte::unique_byte_buffer_t pdu2 = srslte::allocate_unique_buffer(*pool);
    users[rnti_last].send_connection_reconf_replay(std::move(pdu2));
  }
}
void rrc::rrc_send_rrc_reconf_downgraded(uint16_t rnti2){
  if(rnti_last != 0) {
    srslte::unique_byte_buffer_t pdu2 = srslte::allocate_unique_buffer(*pool);
    users[rnti_last].send_connection_reconf_downgraded(std::move(pdu2));
  }
}


void rrc::rrc_send_connection_setup(uint16_t rnti2){
  users[rnti_last].send_connection_setup();
}



void rrc::rrc_send_connection_setup_plain_header(uint16_t rnti2){
  users[rnti_last].send_connection_setup_plain_header();
}
...
bool rrc::setup_ue_ctxt_replay(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTREPLAY_STRUCT *msg)
{
  pthread_mutex_lock(&user_mutex);
  ...
  // Send RRC security mode command
  users[rnti].send_security_mode_command_replay();
...
bool rrc::setup_ue_ctxt_downgraded(uint16_t rnti, LIBLTE_S1AP_MESSAGE_INITIALCONTEXTSETUPREQUESTDOWNGRADED_STRUCT *msg)
{
  pthread_mutex_lock(&user_mutex);
  ...
  uint8_t key[32];
  liblte_pack(msg->SecurityKey.buffer, LIBLTE_S1AP_SECURITYKEY_BIT_STRING_LEN, key);
  users[rnti].set_security_key_plain(key, LIBLTE_S1AP_SECURITYKEY_BIT_STRING_LEN/8);
...
void rrc::disable_integrity(uint16_t rnti, uint32_t lcid)
{
  pdcp->disable_integrity(rnti, lcid);
}

void rrc::disable_encryption(uint16_t rnti, uint32_t lcid)
{
  pdcp->disable_encryption(rnti, lcid);
}
/*******************************************************************************
  RRC thread
*******************************************************************************/
...
void rrc::ue::handle_rrc_con_reest_req(rrc_conn_reest_request_s* msg, srslte::unique_byte_buffer_t pdu)
{
  ...
  reest.next_hop_chaining_count = 0;
  // mac-MainConfig
  ...
  parent->pdcp->config_security(rnti, 1, k_rrc_enc, k_rrc_int, k_up_enc, cipher_algo, integ_algo);
  parent->pdcp->enable_integrity(rnti, 1);
...
{
  parent->rrc_log->console("RRCConnectionSetupComplete transaction ID: %d\n", msg->rrc_transaction_id);
  ...
  state = RRC_STATE_WAIT_FOR_CON_RECONF_COMPLETE;
  parent->rrc_log->console("Response: rrc_connection_setup_complete\n");
...
void rrc::ue::handle_rrc_reconf_complete(rrc_conn_recfg_complete_s* msg, srslte::unique_byte_buffer_t pdu)
{
  ...
  uint8_t response[24] = "rrc_reconf_complete\n";
  uint8_t size = 24;
...
void rrc::ue::handle_security_mode_complete(security_mode_complete_s* msg)
{
  ...
  uint8_t response[28] = "rrc_security_mode_complete\n";
  uint8_t size = 28;
  parent->s1ap->notify_response(response,size);
...
bool rrc::ue::handle_ue_cap_info(ue_cap_info_s* msg)
{
  ...
  uint8_t response[17] = "rrc_ue_cap_info\n";
  uint8_t size = 17;
  parent->s1ap->notify_response(response,size);
...
void rrc::ue::set_security_key_plain(uint8_t* key, uint32_t length)
{
  memcpy(k_enb, key, length);
  parent->rrc_log->info_hex(k_enb, 32, "Key eNodeB (k_enb)");
  // Selects security algorithms (cipher_algo and integ_algo) based on capabilities and config preferences
...
void rrc::ue::send_connection_reest_req()
{
  uint8_t response[26] = "rrc_connection_reest_req\n";
  uint8_t size = 26;
  parent->s1ap->notify_response(response,size);
}
...
void rrc::ue::send_connection_setup_plain_header(bool is_setup)
{
  dl_ccch_msg_s dl_ccch_msg;
  dl_ccch_msg.msg.set_c1();
...
void rrc::ue::send_connection_reest_plain_text()
{
  send_connection_setup_plain_header(false);
}
...
void rrc::ue::send_connection_reconf_replay(srslte::unique_byte_buffer_t pdu)
{
  dl_dcch_msg_s dl_dcch_msg;
  ...
  send_dl_dcch_replay(&dl_dcch_msg, std::move(pdu));

  state = RRC_STATE_WAIT_FOR_CON_RECONF_COMPLETE;
}

void rrc::rrc_send_ue_capability_enquiry(uint16_t rnti2){
  if (rnti_last != 0) {
    users[rnti_last].send_ue_cap_enquiry();

  }
}


void rrc::rrc_send_connection_reestablish(uint16_t rnti2){
  if (rnti_last != 0) {
    users[rnti_last].send_connection_reest();

  }
}


void rrc::rrc_send_connection_reestablish_plain_text(uint16_t rnti2){
  if (rnti_last != 0) {
    users[rnti_last].send_connection_reest_plain_text();
...
void rrc::ue::send_connection_reconf_downgraded(srslte::unique_byte_buffer_t pdu)
{
  dl_dcch_msg_s dl_dcch_msg;
  dl_dcch_msg.msg.set_c1().set_rrc_conn_recfg().crit_exts.set_c1().set_rrc_conn_recfg_r8();
  dl_dcch_msg.msg.c1().rrc_conn_recfg().rrc_transaction_id = (uint8_t)((transaction_id++) % 4);
  ...
  parent->pdcp->add_bearer(rnti, 2, pdcp_cnfg);
  parent->pdcp->config_security(rnti, 2, k_rrc_enc, k_rrc_int, k_up_enc, cipher_algo, integ_algo);
  parent->pdcp->disable_integrity(rnti, 1);
  parent->pdcp->disable_encryption(rnti, 1);
...
void rrc::ue::send_security_mode_command_replay()
{
  dl_dcch_msg_s dl_dcch_msg;
  security_mode_cmd_s* comm = &dl_dcch_msg.msg.set_c1().set_security_mode_cmd();
  comm->rrc_transaction_id  = (uint8_t)((transaction_id++) % 4);
  ...
  send_dl_dcch_replay(&dl_dcch_msg);
}
...
void rrc::ue::send_dl_dcch_replay(dl_dcch_msg_s* dl_dcch_msg, srslte::unique_byte_buffer_t pdu)
{
  if (!pdu) {
    pdu = srslte::allocate_unique_buffer(*pool);
  }
  if (pdu) {
    ...
    sprintf(buf, "SRB1 - rnti=0x%x", rnti);
    parent->log_rrc_message(buf, Tx, pdu.get(), *dl_dcch_msg);
    parent->pdcp->write_sdu_replay(rnti, RB_ID_SRB1, std::move(pdu));
  } else {
    parent->rrc_log->error("Allocating pdu\n");
  }
}
